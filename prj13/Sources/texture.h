...
public:
    ...
    bool Load();
    virtual ColorA Sample(const Point3 &uvw) const;
    virtual bool SetViewportTexture() const;
private:
    std::vector<cyColor32> data;
    int width, height;
...
{
public:
    TextureChecker() : color1(0,0,0,1), color2(1,1,1,1), viewportTextureID(0) {}
    void SetColor1(const ColorA &c) { color1=c; }
    void SetColor2(const ColorA &c) { color2=c; }
    virtual ColorA Sample(const Point3 &uvw) const;
    virtual bool SetViewportTexture() const;
private:
    ColorA color1, color2;
    mutable unsigned int viewportTextureID;
};
