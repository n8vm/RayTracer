...
public:
    DirectLight() : intensity(0,0,0), direction(0,0,1) {}
		virtual Color Illuminate(const Point3 &p, const Point3 &N) const { return Shadow({ Ray(p,-direction), Ray(), Ray() }) * intensity; }
...
public:
    PointLight() : intensity(0,0,0), position(0,0,0) {}
		virtual Color Illuminate(const Point3 &p, const Point3 &N) const { return Shadow({ Ray(p,position - p), Ray(), Ray() }, 1) * intensity; }
