...
bool LoadPPM(FILE *fp, int &width, int &height, std::vector<Color32> &data)
{
	...
	fread(data.data(), sizeof(Color32), width*height, fp);
...
bool TextureFile::Load()
{
    ...
    if ( strncmp(ext,"png",3) == 0 ) {
        ...
        unsigned int w, h;
        unsigned int error = lodepng::decode(d,w,h,name,LCT_RGBA);
        if ( error == 0 ) {
            width = w;
            height = h;
            data.resize(width*height);
            memcpy( data.data(), d.data(), width*height*4 );
        }
        success = (error == 0);
...
ColorA TextureFile::Sample(const Point3 &uvw) const
{
    if ( width + height == 0 ) return ColorA(0,0,0,1);
    ...
    return  data[iy *width+ix ].ToColorA() * ((1-fx)*(1-fy)) +
            data[iy *width+ixp].ToColorA() * (   fx *(1-fy)) +
            data[iyp*width+ix ].ToColorA() * ((1-fx)*   fy ) +
            data[iyp*width+ixp].ToColorA() * (   fx *   fy );
}
 
//-------------------------------------------------------------------------------
 
ColorA TextureChecker::Sample(const Point3 &uvw) const
{
    Point3 u = TileClamp(uvw);
    if ( u.x <= 0.5f ) {
